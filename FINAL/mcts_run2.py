...
def heuristics(distance_matrix):
    ...
    # Average distance to calculate dissimilarity
    average_distance = np.mean(distance_matrix)

    # Calculate dissimilarity and connectivity strength
    dissimilarity_matrix = np.abs(distance_matrix - average_distance)
    connectivity_matrix = 1 / (pairwise_distances(distance_matrix, metric='euclidean') + 1e-5)
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                ...
                # Connection strength influenced by dissimilarity and connectivity
                connection_strength = (1 / (degrees[i] + degrees[j] + 1e-5)) * (1 / (dissimilarity_matrix[i, j] + 1e-5)) * connectivity_matrix[i, j]

                # Combine edge length and connection strength with penalties
                heuristics_matrix[i, j] = edge_length + connection_strength * edge_length
