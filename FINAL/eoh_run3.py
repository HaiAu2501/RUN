    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                ...
                # Calculate heuristic as the sum of minimum edges from both nodes
                min_edge_from_i = np.min(distance_matrix[i][distance_matrix[i] != np.inf]) if np.any(distance_matrix[i] != np.inf) else 0
                min_edge_from_j = np.min(distance_matrix[j][distance_matrix[j] != np.inf]) if np.any(distance_matrix[j] != np.inf) else 0
                heuristics_matrix[i][j] = edge_cost - (min_edge_from_i + min_edge_from_j)
